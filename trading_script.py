...
try:
    import pandas_datareader.data as pdr  # type: ignore[import]
...
def _read_json_file(path: Path) -> Optional[Dict]:
    ...
    try:
        ...
        logger.warning(
            "tickers.json present but malformed: %s -> %s. Falling back to defaults.",
            path,
            exc,
        )
        return None
        ...
        logger.warning(
            "Unable to read tickers.json (%s): %s. Falling back to defaults.",
            path,
            exc,
        )
        return None
...
def load_benchmarks(script_dir: Path | None = None) -> List[str]:
    ...
    if not isinstance(benchmarks, list):
        logger.warning(
            "tickers.json at %s missing 'benchmarks' array. Falling back to defaults.",
            cfg_path,
        )
        return DEFAULT_BENCHMARKS.copy()
...
def last_trading_date(today: datetime | None = None) -> pd.Timestamp:
    ...
    if dt.weekday() == 5:  # Sat -> Fri
        ...
        logger.info(
            "Script running on Saturday - using Friday's data (%s) instead of today's date",
            friday_date.date(),
        )
        return friday_date
    if dt.weekday() == 6:  # Sun -> Fri
        ...
        logger.info(
            "Script running on Sunday - using Friday's data (%s) instead of today's date",
            friday_date.date(),
        )
        return friday_date
    return dt.normalize()
    ...
    if isinstance(df.columns, pd.MultiIndex):
        try:
            ...
            if len(set(df.columns.get_level_values(1))) == 1:
                ...
                df = df.copy()
                df.columns = [
                    "_".join(map(str, t)).strip("_") for t in df.columns.to_flat_index()
                ]
        except Exception:
            df = df.copy()
            df.columns = [
...
def _stooq_download(
    ...
    try:
        if not _HAS_PDR:
            return pd.DataFrame()
        import pandas_datareader.data as pdr_local  # type: ignore[import]
...
def _weekend_safe_range(period: str | None, start: Any, end: Any) -> tuple[pd.Timestamp, pd.Timestamp]:
    ...
    start_ts = (end_trading - pd.Timestamp(days=days)).normalize()
...
def set_data_dir(data_dir: Path) -> None:
    ...
    TRADE_LOG_CSV_PATH = DATA_DIR / TRADE_LOG_CSV_FILE
    logger.info(
        "Data directory configured - Portfolio CSV: %s, Trade Log CSV: %s",
        PORTFOLIO_CSV_PATH,
        TRADE_LOG_CSV_PATH,
    )
...
def _ensure_df(
    ...
    if isinstance(portfolio, (dict, list)):
        ...
        if df.empty:
            logger.debug(
                "Creating empty portfolio DataFrame with proper column structure"
            )
            df = pd.DataFrame(
...
Would you like to log a manual trade? Enter 'b' for buy, 's' for sell, "u" to update a stoploss, or press Enter to continue: """
            ...
            if action == "u":
                ...
                portfolio_df.loc[portfolio_df["ticker"] == ticker, "stop_loss"] = (
                    new_stoploss
                )
            ...
            if action == "b":
                ticker = input("Enter ticker symbol: ").strip().upper()
                order_type = input(
                    "Order type? 'm' = market-on-open, 'l' = limit: "
                ).strip().lower()
                ...
                if order_type == "m":
                    ...
                    if data.empty:
                        print(
                            f"MOO buy for {ticker} failed: no market data available (source={fetch.source})."
                        )
                        continue

                    o = float(data["Open"].iloc[-1]) if "Open" in data else float(
                        data["Close"].iloc[-1]
                    )
                    ...
                    if notional > cash:
                        print(
                            f"MOO buy for {ticker} failed: cost {notional:.2f} exceeds cash {cash:.2f}."
                        )
                        continue
                    ...
                    rows = portfolio_df.loc[
                        portfolio_df["ticker"].astype(str).str.upper()
                        == ticker.upper()
                    ]
                    if rows.empty:
                        new_trade = {
                    ...
                    cash -= notional
                    print(
                        f"Manual BUY MOO for {ticker} filled at ${exec_price:.2f} ({fetch.source})."
                    )
                    continue
            ...
            if action == "s":
                try:
                    ...
                    if ticker not in portfolio_df["ticker"].values:
                        print(
                            f"Manual sell for {ticker} failed: ticker not in portfolio."
                        )
                        continue
                    sell_order_type = input(
                        "Order type? 'm' = market-on-open, 'l' = limit: "
                    ).strip().lower()
                    ...
                    if sell_order_type == "l":
                        ...
                        # For MOO, approximate by asking user for expected open price
                        sell_price = float(
                            input(
                                "Enter expected market-on-open execution price: "
                            ).strip()
                        )
                    else:
                        print("Unknown order type. Use 'm' or 'l'.")
    ...
    for _, stock in portfolio_df.iterrows():
        ...
        cost_basis = float(stock["cost_basis"]) if not pd.isna(stock["cost_basis"]) else (
            cost * shares
        )
    ...
    if PORTFOLIO_CSV_PATH.exists():
        ...
        if existing.empty:
            ...
            try:
                ...
                logger.debug(
                    "Reindexing existing CSV to match df_out columns failed; falling back to concat with sort=False"
                )
...
def daily_results(chatgpt_portfolio: pd.DataFrame, cash: float) -> None:
    ...
    # go back enough to capture 2 sessions even around holidays
    ...
    for stock in portfolio_dict + benchmark_entries:
        ...
        try:
            fetch = download_price_data(
                ticker, start=start_d, end=(end_d + pd.Timedelta(days=1)), progress=False
            )
            data = fetch.df
    ...
    totals["Date"] = pd.to_datetime(
        totals["Date"], format="mixed", errors="coerce"
    )  # tolerate ISO strings
    totals = totals.sort_values("Date")

    final_equity = float(totals.iloc[-1]["Total Equity"])
    equity_series = (
        totals.set_index("Date")["Total Equity"].astype(float).sort_index()
    )
    ...
    sharpe_annual = (
        ((mean_daily - rf_daily) / std_daily) * np.sqrt(252) if std_daily > 0 else np.nan
    )
    sortino_period = (
    ...
    print(
        f"{'Sharpe Ratio (period):':32} {fmt_or_na(sharpe_period, '{:.4f}'):>15}"
    )
    print(
        f"{'Sharpe Ratio (annualized):':32} {fmt_or_na(sharpe_annual, '{:.4f}'):>15}"
    )
    print(
        f"{'Sortino Ratio (period):':32} {fmt_or_na(sortino_period, '{:.4f}'):>15}"
    )
    print(
            ...
            if vol_str != "—":
                ...
                if price_str != "—":
                    ...
                    if dollar_volume < 50000:
                        print(
                            f"  {ticker_str}: SEVERE liquidity - only ${dollar_volume:,.0f} traded today"
                        )
                        warnings_found = True
                    elif dollar_volume < 100000:
                        print(
                            f"  {ticker_str}: Low liquidity - ${dollar_volume:,.0f} traded today"
                        )
                        warnings_found = True
        except Exception:
            pass
...
def load_latest_portfolio_state(
    ...
    try:
        ...
        logger.info(
            "Portfolio CSV not found at %s, starting with an empty portfolio.",
            PORTFOLIO_CSV_PATH,
        )
        # Treat as empty portfolio; downstream logic will prompt for starting equity.
        df = pd.DataFrame(
    ...
    if df.empty:
        ...
        if starting_equity_override is not None:
            ...
            if parsed is None:
                raise ValueError(
                    "Provided starting equity is invalid. Please pass a positive number."
                )
        ...
        if not sys.stdin.isatty():
            print(
                "Error: No starting equity provided and stdin is not interactive. Provide --starting-equity or run interactively."
            )
            sys.exit(2)
        ...
        while True:
            ...
            if parsed is not None:
                ...
                break
            print(
                "Invalid amount. Enter a positive number (commas, underscores, $ prefix allowed). Try again."
            )
    ...
    latest_tickers_records = latest_tickers.reset_index(drop=True).to_dict(
        orient="records"
    )
    ...
    return latest_tickers_records, cash
...
def main(
    ...
    chatgpt_portfolio, cash = load_latest_portfolio_state(
        starting_equity_override=starting_equity_override
    )
...
if __name__ == "__main__":
    ...
    if args.log_level:
        ...
        logging.getLogger("peewee").setLevel(
            logging.WARNING
        )  # ignore constant info messages from PeeWee
        _log_initial_state()
